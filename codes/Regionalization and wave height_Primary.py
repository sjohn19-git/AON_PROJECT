...
with open(("median and mean"+str(time_frame[0])+"-"+str(time_frame[-1])+"primary.pkl"), 'rb') as f:  # Python 3: open(..., 'wb')
...
axs[1].set_title("mean PSD over the stations in the region_7 hour median Filtered 10 Hz-15.4 Hz")
...
#def wave_val(grid,wlat1,wlat2,wlon1,lon2):
#     m_lon1=180+(180+wlon2)
#     m_lon2=180+(180+wlon1)
#     ilat1=np.where(grid.lat==wlat1)[0][0]
#     ilat2=np.where(grid.lat==wlat2)[0][0]
...
#         median_timeseries[median_timeseries==0]=np.mean(median_timeseries[0,ele,1:])
#         mean_timeseries[mean_timeseries==0]=np.mean(mean_timeseries[0,ele,1:])
#         sum_array=sum_array+median_timeseries[0,ele,1:]
#         sum_mean=sum_mean+mean_timeseries[0,ele,1:]
#     return sum_array,sum_mean

# os.chdir("/Users/sebinjohn/AON_PROJECT/Data/median_Power_time_series")
# with open(("median and mean"+str(time_frame[0])+"-"+str(time_frame[-1])+"primary.pkl"), 'wb') as f:  # Python 3: open(..., 'wb')
...
for i in range(len(wista)):
    if wista[i]==1:
        lab=stationo[i-1][6:-4]
        #if lab=="S32K" or lab=="V35K":
        axs.plot(time_framem,mean_timeseries_plo[0,i,1:],label=lab)
axs.xaxis.set_major_formatter(date_format)
axs.set_title("raw-PSD of stations in 10 Hz-15.4 Hz band")
axs.set_ylabel("PSD")

#axs.set_xlim([18210,18214])
#axs.set_ylim([-165,-115])
axs.legend()
axs.xaxis.set_major_locator(mdates.DayLocator(interval=1))

#axs.set_ylim([-160,-110])
axs.set_xlim([18221,18225])
axs.vlines(18210,-160,-110,colors="red",linestyles='solid')
axs.vlines(18212,-160,-110,colors="red",linestyles='solid')
axs.vlines(18221,-160,-110,colors="red",linestyles='solid')
axs.vlines(18225,-160,-110,colors="red",linestyles='solid')
